...
2. The app ZIP comes from https://bulk-keygen.up.railway.app/ using Download ZIP. Do not invent mirrors. Extract into a new folder, never overwrite keys. The ZIP contains the app, not my key or wallet session.
...
Explain the manual flow: intended network (testnet first), owner wallet, create/select the intended subaccount, generate key, privately save backup, review account and full agent public key, approve signature myself, save updated request, explicitly submit. Do not broaden scope to the main account. A signature is not BULK confirmation; wait for status. A timeout is not rejection. Retry only the current, unchanged request from the latest signed backup; a key-only backup cannot recover a lost signing request or automatically authorize a new one. Never replay an old registration after revoking access.
Backups now use version 2 JSON internally, preserving the current request and signature history. Older plain JSON and encrypted version 1 backups remain supported. Historical signatures are not cancelled and never replay automatically. Stop waiting only stops the local wait: wallet requests may still complete and submitted actions may still take effect. A saved subaccount-creation request can be closed locally and restored from its file.
For subaccounts, CLI direct_* statuses describe the selected account's own agent membership. Explain account, directMembership, inheritedMembership and effectiveAccess together; null means unknown. direct_revoked does not rule out inherited parent-account access.
