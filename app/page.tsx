    ...
    <section className="card" aria-label="Agent key manager">
      ...
      <div className="key-section stack">
        ...
        {notice && <div role="status" className="message">{notice}</div>}
          ...
          <section className="stack" aria-label="Backups">
            <div className="field"><button className={`btn ${stage === 'created' && !saved || stage === 'signed' && !recoverySaved ? 'primary' : ''} full`} disabled={!!busy} onClick={() => setDialog('export')}><LockKeyhole size={17}/> Save encrypted backup</button><p className="hint">Recommended for storage. Encrypts your key and recovery data with your password.</p></div>
            <div className="field"><button className="btn full" disabled={!!busy} onClick={downloadKey}><Download size={17}/> {downloadLabel}</button><p className="hint">Unencrypted JSON. Anyone with this file can use your key.</p></div>
          </section>
    ...
    <Dialog open={dialog === 'export' || dialog === 'import'} onOpenChange={open => { if (!open && !busyRef.current) closeDialog(); }}>
      <DialogContent className="vault-dialog" showCloseButton={!busy}>
        ...
        <form className="stack" onSubmit={event => { event.preventDefault(); void vaultAction(); }}>
          ...
          {dialog === 'export' && <p className="hint">Protects the saved file. It cannot protect keys opened on a compromised device or page. Existing unencrypted copies stay unprotected.</p>}
